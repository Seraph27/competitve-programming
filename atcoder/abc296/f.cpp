...
// Contest: AtCoder Beginner Contest 296
// URL: https://atcoder.jp/contests/abc296/tasks/abc296_f?utm_source=chatgpt.com
// Time Limit: 2000
// Start: Mon Aug 10 18:23:52 2026
// atcoder
#ifdef MISAKA
...
void shiina_mashiro() {
    ...
    auto inv = [&](auto &vec) -> int {
        fenwick<int> fw(n + 1);
        int res = 0;
        for(int i = 0; i < n; i++) {
            fw.update(vec[i], fw.query(vec[i], vec[i]) + 1);
            res += fw.query(vec[i] + 1, n + 1);
        }
        return res;
    };

    int mx = max(*max_element(all(a)), *max_element(all(b)));
    vector<int> freq_a(mx + 1), freq_b(mx + 1);
    for(int i = 0; i < n; i++) {
        freq_a[a[i]]++;
        freq_b[b[i]]++;
    }
    bool ok = true;
    for(int i = 0; i <= mx; i++) if(freq_a[i] != freq_b[i]) {
        ok = false;
        break;
    }
