...
// URL: https://atcoder.jp/contests/abc465/tasks/abc465_e
// Time Limit: 2000
// Start: Sat Jul  4 22:50:43 2026
// atcoder
#ifdef MISAKA
...
void shiina_mashiro() {
                ...
                for(int mod = 0; mod < 3; mod++) {
                    for(int has = 0; has < 2; has++) {
                        for(int msk = 0; msk < (1 << 10); msk++) {
                            mint curr = dp[i][tight][lz][mod][has][msk];
                            ...
                            for(int o = 0; o <= lim; o++) {
                                if(lz && !o) {
                                    dp[i + 1][tight && (o == lim)][lz && !o][mod][has][msk] += curr; 
                                } else {
                                    auto nmod = (mod + o) % 3;
                                    auto nhas = has || (o == 3);
                                    auto nmsk = msk | (1 << o);
                                    dp[i + 1][tight && (o == lim)][lz && !o][nmod][nhas][nmsk] += curr; 
                                }
                            }
                        }
                    }
    ...
    for(int msk = 0; msk < (1 << 10); msk++) for(int tight = 0; tight < 2; tight++) for(int lz = 0; lz < 2; lz++) {
        if(__builtin_popcount(msk) == 3) continue;
        ans += dp[n][tight][lz][0][0][msk];
    }
    
    for(int mod = 1; mod <= 2; mod++) for(int msk = 0; msk < (1 << 10); msk++) for(int tight = 0; tight < 2; tight++) for(int lz = 0; lz < 2; lz++) {
        if(__builtin_popcount(msk) == 3) continue;
        ans += dp[n][tight][lz][mod][1][msk];
    }

    for(int mod = 1; mod <= 2; mod++) for(int msk = 0; msk < (1 << 10); msk++) for(int tight = 0; tight < 2; tight++) for(int lz = 0; lz < 2; lz++) {
        if(__builtin_popcount(msk) == 3) ans += dp[n][tight][lz][mod][0][msk];
    }
