...
// Contest: AtCoder Beginner Contest 250
// URL: https://atcoder.jp/contests/abc250/tasks/abc250_e
// Time Limit: 4000
// Start: Sun Aug  2 00:26:53 2026
// atcoder
#ifdef MISAKA
...
void shiina_mashiro() {
    ...
    set<int> sa, sb;
    map<int, int> mp;
    vc pref_a(n + 1), pref_b(n + 1);
    for(int i = 0; i < n; i++) {
        int z = 0, z2 = 0;
        if(sa.insert(a[i]).second) {
            if(mp.count(a[i])) z = mp[a[i]];
            else z = rng(), mp[a[i]] = z;
        }
        if(sb.insert(b[i]).second) {
            if(mp.count(b[i])) z2 = mp[b[i]];
            else z2 = rng(), mp[b[i]] = z2;
        }
        pref_a[i + 1] += pref_a[i] + z;
        pref_b[i + 1] += pref_b[i] + z2;
    }
    debug(pref_a, pref_b);
    ...
    for(int i = 0; i < q; i++) {
        int x, y; cin >> x >> y;
        if(pref_a[x] == pref_b[y]) cout << "Yes" << nl;
        else cout << "No" << nl;
    }
}
