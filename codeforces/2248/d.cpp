// Problem: D. Good Pair Queries
// Contest: Codeforces Round 1113 (Div. 2)
// URL: https://codeforces.com/contest/2248/problem/D
// Time Limit: 2000
// Start: Sat Aug  1 08:41:05 2026
// multitest
#ifdef MISAKA
...
void shiina_mashiro() {
    ...
    vc a(n + 1), b(n + 1), c(n + 1), d(n + 1);
    for(int i = 0; i < n; i++) {
        a[i + 1] += a[i] + (s[i] == '0' and t[i] == '1');
        b[i + 1] += b[i] + (s[i] == '1' and t[i] == '0');
        c[i + 1] += c[i] + (s[i] == '0' and t[i] == '0');
        d[i + 1] += d[i] + (s[i] == '1' and t[i] == '1');
    }

    for(;q--;) {
        int l, r; cin >> l >> r;
        cout << ((abs(b[r] - b[l - 1] - a[r] + a[l - 1]) <= c[r] - c[l - 1] + d[r] - d[l - 1]) ? "Yes" : "No") << nl;
    }
}
