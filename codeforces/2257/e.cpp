...
// Time Limit: 2000
// Start: Mon Aug 17 09:09:45 2026
// multitest
#ifdef MISAKA
...
void shiina_mashiro() {
    ...
    vvc a(n), b(n);
    vector<vector<ar<int, 3>>> group(n);
    for(int i = 0; i < n; i++) {
        int m; cin >> m;
        a[i].resize(m);
        b[i].resize(m);
        ...
        for(int j = 0; j < m; j++) {
            ckmax(need, a[i][j] - gain);
            gain += b[i][j] - a[i][j];
            if(gain > 0) {
                group[i].pb({need, gain, j + 1});
                need = 0;
                gain = 0;
            }
        }
    ...
    for(int i = 0; i < n; i++) if(sz(group[i])) pq.push({group[i][0][0], i, 0});

    while(sz(pq) and pq.top()[0] <= x) {
        auto [need, i, j] = pq.top(); pq.pop();
        ...
        if(j < sz(group[i])) { //
            pq.push({group[i][j][0], i, j});
            debug(group[i][j][0], i, j);
        }
    }
