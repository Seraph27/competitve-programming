...
// Time Limit: 2000
// Start: Mon Aug 17 08:27:25 2026
// multitest
#ifdef MISAKA
...
void shiina_mashiro() {
    ...
    debug(pref);
    auto f = [&](int x) { //only care about X
        auto i = lower_bound(all(divs), x) - divs.begin();
        return pref[i - 1] + (x - divs[i- 1]) * (s / divs[i]);
    };
    ...
    for(;q--;) {
        ...
        auto idx = upper_bound(all(divs), s / y) - divs.begin() - 1;
        int p = divs[idx];
        if(x <= p) ans = x * y;
        else ans = p * y + f(x) - f(p);
